...
export const EconomistFacet: React.FC = () => {
  const [subsidyReformPct, setSubsidyReformPct] = useState(45);

  // Dynamic Macroeconomic Calculations (Based on Pakistan Power Sector / IMF FPP.1x Model)
  const annualSavingsPKR = Math.round((subsidyReformPct / 100) * 420); // out of ~420B PKR annual drain
  const protectedFamiliesM = (subsidyReformPct * 0.16).toFixed(1); // targeted transfer expansion
  ...
  return (
    <div className="glass-quantum rounded-3xl p-6 sm:p-10 border border-mint-500/30 text-slate-100 shadow-2xl space-y-8">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-mint-500/20 pb-6">
        <div>
          ...
          <p className="text-sm text-slate-300 font-mono mt-1">
            IMF FPP.1x (A+ Grade) • IDB Data for Policy (95%) • MIT DEDP Candidate
          </p>
        </div>
        <div className="px-4 py-2 rounded-xl bg-obsidian-900 border border-mint-500/40 text-mint-300 text-xs font-mono shadow-md">
          Credential Key: IMF-FPP1x-A+
        </div>
      </div>

      {/* Interactive Energy Subsidy Reform Simulator */}
      <div className="p-6 rounded-2xl bg-obsidian-900/80 border border-mint-500/30 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <span className="text-xs font-mono text-mint-400">
            IMF ESRx & MicroMasters Methodology
          </span>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs text-slate-200">
        ...
        <div className="p-4 rounded-xl bg-obsidian-900/60 border border-mint-500/15 space-y-1">
          <div className="font-semibold text-mint-400 font-mono">MIT MicroMasters DEDP Candidate</div>
          <p className="text-slate-300">
